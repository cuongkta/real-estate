...
defmodule RealEstateWeb.UserControllerTest do
  ...
  describe "update user" do
    ...
    test "renders user when data is valid", %{conn: conn, user: %User{id: id} = user} do
      ...
      response = put conn, api_v1_user_path(conn, :update, user), user: @update_attrs
      assert %{"id" => ^id} = json_response(response, 200)
      ...
      assert json_response(conn, 200)["email"] == "b@gmail.com"
    end
  ...
  describe "delete user" do
    ...
    test "deletes chosen user", %{conn: conn, user: user} do
      ...
      response = delete conn, api_v1_user_path(conn, :delete, user)
